...
async function pullFromGitHub(){
  ...
  try{
    ...
    const res=await fetch(url,{headers:{Authorization:`token ${ghSettings.token}`,Accept:'application/vnd.github.v3+json'}});
...
async function testGitHubConnection() {
  ...
  if (!ghSettings.token) { showGhStatus('error', '❌ Please enter your GitHub Personal Access Token first.'); return; }
  if (!ghSettings.owner || !ghSettings.repo) { showGhStatus('error', '❌ Please enter your GitHub username and repository name.'); return; }
  ...
  try {
    ...
    const res = await fetch(url, { headers: { Authorization: `token ${ghSettings.token}`, Accept: 'application/vnd.github.v3+json' } });
    if (res.status === 200) {
      showGhStatus('success', `✅ Connected! Found "${ghSettings.filePath}" in ${ghSettings.owner}/${ghSettings.repo} on branch "${ghSettings.branch}".`);
    } else if (res.status === 404) {
      showGhStatus('error', `❌ File not found. Upload "data/products.json" to your repo first, or check your username/repo/branch spelling.`);
    } else if (res.status === 401) {
      showGhStatus('error', '❌ Authentication failed. Make sure your token has "repo" scope and is not expired.');
    } else {
      showGhStatus('error', `❌ GitHub API error: ${res.status} ${res.statusText}`);
    }
  } catch (e) { showGhStatus('error', '❌ Network error: ' + e.message); }
}

/* ── Publish to GitHub ──────────────────────────────────── */
async function publishToGitHub() {
  // Sync from form fields if the GitHub panel is currently visible
  ...
  if (ghPanelVisible) {
    ...
    ghSettings.branch   = document.getElementById('ghBranch')?.value.trim()   || ghSettings.branch;
    ghSettings.filePath = document.getElementById('ghFilePath')?.value.trim() || ghSettings.filePath;
  } else {
    // Load from storage when publishing from other panels
    loadGitHubSettings();
  }

  if (!ghSettings.token || !ghSettings.owner || !ghSettings.repo) {
    toast('❌ Configure GitHub Settings before publishing.', 'error'); switchSection('github'); return;
  }
  ...
  try {
    ...
    const headers  = { Authorization: `token ${ghSettings.token}`, Accept: 'application/vnd.github.v3+json', 'Content-Type': 'application/json' };

    // Get current SHA (needed to update existing file)
    let sha = null;
    const getRes = await fetch(`${fileUrl}?ref=${ghSettings.branch}`, { headers });
    if (getRes.ok) { const fileData = await getRes.json(); sha = fileData.sha; }
    else if (getRes.status !== 404) { throw new Error(`GitHub GET error: ${getRes.status}`); }

    // Push updated file
    const body = { message: `Admin: Update products.json — ${new Date().toISOString()}`, content, branch: ghSettings.branch };
    if (sha) body.sha = sha;
    ...
    showGhStatus('success', '✅ Published to GitHub! Changes will be live on GitHub Pages in ~30 seconds.');
    toast('✅ Published to GitHub successfully!', 'success', 6000);

  } catch (e) {
    dismissToast(tid);
    showGhStatus('error', '❌ Publish failed: ' + e.message);
    toast('❌ GitHub publish failed: ' + e.message, 'error', 8000);
  }
}
...
function switchSection(sec){
  ...
  if(sec==='github') loadGitHubSettings();
}
