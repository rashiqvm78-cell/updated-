...
/* ══════════════════════════════════════════════════════════
   GITHUB SETTINGS — full GitHub API publish method
   (alternative to Cloudflare — works with GitHub Pages)
══════════════════════════════════════════════════════════ */
...
async function testGitHubConnection() {
  ...
  if (!ghSettings.token) {
    showGhStatus('error', '❌ Please enter your GitHub Personal Access Token first.');
    return;
  }
  if (!ghSettings.owner || !ghSettings.repo) {
    showGhStatus('error', '❌ Please enter your GitHub username and repository name.');
    return;
  }
  ...
  try {
    ...
    showGhStatus('error', '❌ Network error: ' + e.message);
  }
}
...
async function publishToGitHub() {
  ...
  if (!ghSettings.token || !ghSettings.owner || !ghSettings.repo) {
    showGhStatus('error', '❌ Fill in all GitHub settings and save before publishing.');
    return;
  }

  // Auto-save any open forms
  if (!document.getElementById('panel-hero')?.classList.contains('hidden'))  saveHero();
  if (!document.getElementById('panel-stats')?.classList.contains('hidden')) saveStats();

  showGhStatus('loading', '⏳ Publishing to GitHub…');
...
/* ── Patch switchSection to handle GitHub tab ───────────── */
const _baseSwitchSection = switchSection;
switchSection = function(sec) {
  _baseSwitchSection(sec);
  // Load GH settings fields when tab opens
  if (sec === 'github') loadGitHubSettings();
  // Fix title for github (not in original titles map)
  if (sec === 'github') {
    const te = document.getElementById('sectionTitle');
    const be = document.getElementById('breadcrumbSpan');
    if (te) te.textContent = 'GitHub Settings';
    if (be) be.textContent = 'GitHub Settings';
  }
};
